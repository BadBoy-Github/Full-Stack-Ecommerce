                ...
                <nav>
                  {
                    user?.role === ROLE.ADMIN && (
                      <Link
                        to={"/admin-panel/all-products"} className="whitespace-nowrap hidden md:block hover:bg-slate-100 p-2 rounded"
            ...
            user?._id && (
              <Link to={"/cart"} className="text-3xl relative cursor-pointer">
                ...
                <div className="bg-red-600 text-white w-5 h-5 rounded-full p-1 flex items-center justify-center absolute -top-2 -right-2">
                  <p className="text-sm z-10">{context?.cartProductCount}</p>
                </div>
              </Link>
            )
          }
